...
export const createStore = <S, A>(name: string, initialState: S, actions: (set: Store<S>["setState"], get: Store<S>["getState"]) => A) =>
    hmr(name, create<S & A>()((set, get) => ({ ...initialState, ...actions(set, get) })))

const hmr = <T>(key: string, store: T): T =>
    // @ts-ignore
    import.meta.env.DEV ? (window as any)[key] ?? ((window as any)[key] = store) : store
