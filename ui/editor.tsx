        ...
        switch (s.statement) {
            ...
            case "INSERT": {
                ...
                return [
                    ...
                    s.textareaValues.flatMap((value, i) => filterDefaults(value, i) ? [parseTextareaValue(value, s.blobValues[i]!, s.dataTypes[i]!)] : []),
...
const DataEditor = (props: { column: string, rows?: number, style?: JSXInternal.CSSProperties, type: EditorDataType, textareaValue: string, onTextareaValueChange: (value: string) => void, blobValue: Uint8Array | null, onBlobValueChange: (value: Uint8Array) => void, tabIndex?: number, class?: string, onTypeChange: (type: EditorDataType) => void }) => {
    ...
    if (props.type === "blob") {
        return <div>
            ...
            <label class="inline-block px-2 cursor-pointer [font-family:inherit] focus:outline focus:outline-2 focus:outline-blue-300 text-fg-secondary bg-secondary hover:bg-secondary-hover">
                <input type="file" class="hidden" onChange={async (ev) => {
                    const file = ev.currentTarget.files?.[0]
                    if (!file) { return }
                    props.onBlobValueChange(new Uint8Array(await file.arrayBuffer()))
                }}></input>
                Upload…
            </label>
            <Button class="ml-1" secondary={true} onClick={async () => {
                const result = await fileTypeFromBuffer(props.blobValue ?? new Uint8Array())
                FileSaver.saveAs(new Blob([props.blobValue ?? new Uint8Array()]), `data.${result?.ext ?? "bin"}`)
            }}>Download…</Button>
        </div>
    }
