...
const checkIfCellIsSelectedButNotEditingInPlace = () =>
    useTableStore.getState().input?.textarea === null ||
    document.querySelector(".single-click") !== null
...
const moveSelectionColumn = (delta: number) => {
    ...
    const columnIndex = Math.max(0, Math.min(visibleColumns.length - 1, visibleColumns.indexOf(state.column)! + delta))
    state.update(state.tableName, visibleColumns[columnIndex]!, state.row)
}
...
export const onKeydown = async (ev: KeyboardEvent) => {
    ...
    try {
        ...
        const isCellSelectedButNotEditingInPlace = checkIfCellIsSelectedButNotEditingInPlace()
        ...
        if (findWidgetFocus && key("Escape")) {
        ...
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && key("Escape")) {
            p()
            await state.discardChanges()
        } else if (!inputFocus && state.statement === "UPDATE" && !isCellSelectedButNotEditingInPlace && key("Escape")) {
            p()
            state.update(state.tableName, state.column, state.row)
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && key("ArrowUp")) {
            p()
            await moveSelectionRow(-1n)
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && key("ArrowDown")) {
            p()
            await moveSelectionRow(1n)
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && key("ArrowLeft")) {
            p()
            moveSelectionColumn(-1)
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && key("ArrowRight")) {
            p()
            moveSelectionColumn(1)
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && (key("Ctrl + ArrowUp") || key("Ctrl + Home"))) {
            p()
            await moveSelectionRow(-useTableStore.getState().paging.numRecords)
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && (key("Ctrl + ArrowDown") || key("Ctrl + End"))) {
            p()
            await moveSelectionRow(useTableStore.getState().paging.numRecords)
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && (key("Ctrl + ArrowLeft") || key("Home"))) {
            p()
            moveSelectionColumn(-useTableStore.getState().tableInfo.length)
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && (key("Ctrl + ArrowRight") || key("End"))) {
            p()
            moveSelectionColumn(useTableStore.getState().tableInfo.length)
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && key("PageUp")) {
            p()
            await moveSelectionRow(-useTableStore.getState().paging.visibleAreaSize)
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && key("PageDown")) {
            p()
            await moveSelectionRow(useTableStore.getState().paging.visibleAreaSize)
        } else if (!inputFocus && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && key("(Shift +) Enter")) {
            p()
            const textarea = useTableStore.getState().input?.textarea
            if (textarea) {
                textarea.classList.remove("single-click")
            } else {
                if (!await useEditorStore.getState().beforeUnmount(true)) { return }
                await moveSelectionRow(key("Shift + Enter") ? -1n : 1n)
            }
        } else if ((!inputFocus || cellInputFocus) && state.statement === "UPDATE" && !isCellSelectedButNotEditingInPlace && key("Enter")) {
            p()
            if (!await useEditorStore.getState().beforeUnmount(true)) { return }
            await moveSelectionRow(1n)
        } else if ((!inputFocus || cellInputFocus) && state.statement === "UPDATE" && !isCellSelectedButNotEditingInPlace && key("Shift + Enter")) {
            p()
            if (!await useEditorStore.getState().beforeUnmount(true)) { return }
            await moveSelectionRow(-1n)
        } else if ((!inputFocus || cellInputFocus) && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && key("Tab")) {
            p()
            moveSelectionColumn(1)
        } else if ((!inputFocus || cellInputFocus) && state.statement === "UPDATE" && !isCellSelectedButNotEditingInPlace && key("Tab")) {
            p()
            if (!await useEditorStore.getState().beforeUnmount(true)) { return }
            moveSelectionColumn(1)
        } else if ((!inputFocus || cellInputFocus) && state.statement === "UPDATE" && isCellSelectedButNotEditingInPlace && key("Shift + Tab")) {
            p()
            moveSelectionColumn(-1)
        } else if ((!inputFocus || cellInputFocus) && state.statement === "UPDATE" && !isCellSelectedButNotEditingInPlace && key("Shift + Tab")) {
            p()
